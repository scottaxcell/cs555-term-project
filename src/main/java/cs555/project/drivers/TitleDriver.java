...
public class TitleDriver extends Driver {
    ...
    private static class TitleMetadata implements Serializable {
        final String title;
        final boolean successful;

        public TitleMetadata(String title, boolean successful) {
            this.title = title;
...
//        JavaRDD<String> rdd = sc.textFile("/s/chopin/a/grad/sgaxcell/cs555-term-project/data/movies_metadata.csv");
        ...
        calculatePopulationMeanAndStdDev(allMoviesWithATitle);
    ...
    private void writeStatisticsToFile(JavaSparkContext sc) {
        ...
        writeMe.add("Movie Title Analysis");
        writeMe.add("====================\n");
        ...
        sc.parallelize(writeMe, 1).saveAsTextFile("TitleAnalysis");
    }

    /**
     * Estimate population mean using sample size
     *
     * @param titleMetadatas
     */
    private void calculatePopulationMeanAndStdDev(List<TitleMetadata> titleMetadatas) {
        titleMetadatas.stream()
            .forEach(titleMetadata -> {
                if (titleMetadata.successful)
                    numSuccessful++;
                numMovies++;
